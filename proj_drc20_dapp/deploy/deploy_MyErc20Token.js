...
const { developmentChains, tokenName, tokenSymbol, totalSupply_, CONFIRMATIONS } = require("../helper-hardhat-config")
...
module.exports = async() => {
    ...
    const myErc20Token = await deploy("MyErc20Token", {
        ...
        args: [tokenName, tokenSymbol, totalSupply_],  // 部署时传入的参数
        log: true,  // 是否需要打印参数
    ...
    if( hre.network.config.chainId == 11155111 && process.env.ETHERSCAN_API ) {
        ...
        await hre.run("verify:verify", {  // hre是运行时环境，其实就是js执行命令行脚本
            address: myErc20Token.address,  // 合约地址
            constructorArguments: [tokenName, tokenSymbol, totalSupply_],  // 输入参数
        });
    }else{
