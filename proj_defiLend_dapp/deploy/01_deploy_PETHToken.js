...
module.exports = async() => {
    ...
    // 对合约进行部署
    console.log("\ndeploy PETHToken start......")
    const myPETHToken = await deploy("PETHToken", {
        contract: "MyERC20Token",  // 指定合约类名
        from: firstAccount,  // 那个账户进行部署
        args: ["PETHToken", "PETH"],  // 部署时传入的参数
        log: true,  // 是否需要打印参数
    ...
    if( hre.network.config.chainId == 11155111 && process.env.ETHERSCAN_API ) {
        ...
        await hre.run("verify:verify", {  // hre是运行时环境，其实就是js执行命令行脚本
            address: myPETHToken.address,  // 合约地址
            constructorArguments: ["PETHToken", "PETH"],  // 输入参数
        });
    }else{
        console.log(`local verify skip`)
    }

    console.log("deploy PETHToken finished.")

}

// 给当前部署的合约一个标签
module.exports.tags = ["all", "PETHToken"]
